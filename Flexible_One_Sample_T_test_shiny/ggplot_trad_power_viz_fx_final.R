...
# Flexible version that allows user to enter whatever values they like
...
two_sided_t_h0_leq <- function(data, test_stat, color)
{
...
one_sided_t_h0_leq <- function(data, test_stat, color)
{
  max_h0 <- max(data$pdf_h0)
  text_size <- 5
  plot_size <- 18
...
two_sided_t_h0_greater <- function(data, test_stat, color)
{
...
one_sided_t_h0_greater <- function(data, neg_test_stat, color)
{
  max_h0 <- max(data$pdf_h0)
  plot_size <- 18
  text_size <- 5
  ...
  ggplot(data) + 
    ...
    annotate("text", x = 0, y = max(data$pdf_h0) + 0.01, label = glue("H0"), size = text_size) +
    annotate("text", x = data[[which.max(data$pdf_hA),1]], y = max(data$pdf_hA) + 0.01, label = glue("HA"), size = text_size) +
    annotate("text", x = neg_test_stat, y = max_h0 + 0.01, label = glue("-Tcrit"), size = text_size) +
    theme_bw(base_size = plot_size) +
    scale_fill_manual(name = "Area Represents:", values = color) +
    theme(legend.position="bottom") 
}
...
# ---------------------------------------- CODE BELOW IS FOR TESTING PURPOSES | Both sides T --------------------------------------------
...
#   } else {
#     # mu0 > muA
#     data <- tibble(x_val = seq(from = ncp - ifelse(ncp > 4, -ncp + 1, 3.5)*(df/(df-2)),
#                                to = 3.5*(df/(df-2)),
#                                by = 0.01),
#                    pdf_h0 = dt(x_val, df),
#                    pdf_hA = dt(x_val, df, ncp)) %>%
