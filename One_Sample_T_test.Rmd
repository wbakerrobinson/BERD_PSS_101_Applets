...
ggplot() +
  ...
  geom_segment(aes(x = 0,
               ...
               color = "#999999",
               lty = "dashed") + # change color
  geom_point(power_data, mapping = aes(x = N, y = power, shape = user_alpha, color = alpha_level)) +
  geom_line(power_data, mapping = aes(x = N, y = power, group = alpha_level, color = alpha_level)) +
  geom_point(power_data, mapping = aes(x = user_N, y = user_power), color = "red", shape = 17) +
  theme_bw() +
  labs(x = "Sample Size",
       y = "Power",
       title = "Power Curve: 1 sample T-test") +
  theme(legend.position="bottom") +
  scale_color_manual(name = "Alpha level:", values = c("#0072B2", "#CCCCCC", "#E69F00", "#F0E442"))
```
